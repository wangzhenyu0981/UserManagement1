    ...
    <style>
        * { box-sizing: border-box; }
        ...
        form input:focus, form select:focus {
            ...
            border-color: #2193b0;
            background: #fff;
            box-shadow: 0 0 0 3px rgba(33,147,176,.18);
        }
        ...
        .btn-submit { background: linear-gradient(135deg, #2193b0, #6dd5ed); }
        .btn-cancel {
            background: #fff;
            color: #555;
            border: 1.5px solid #dbe1ec;
...
<body>
    <div class="container">
        <div class="hero">
            ...
            <div>
                <h1>修改学生信息</h1>
                <div class="sub">学号：<%= s.getSno() %> · 请修改后点击「保存」</div>
            </div>
        </div>
        <form action="<%= ctx %>/DoStudentServlet.do" method="post">
            <input type="hidden" name="id" value="<%= s.getId() %>">

            <label>学号</label>
            <input type="text" name="sno" value="<%= s.getSno() %>" required>

            <label>姓名</label>
            <input type="text" name="sname" value="<%= s.getSname() %>" required>
            ...
            <div class="row">
                ...
                <div>
                    <label>年龄</label>
                    <input type="number" name="age" min="1" max="150" value="<%= s.getAge() %>">
                </div>
            </div>
            ...
            <label>成绩</label>
            <input type="number" name="grade" step="0.01" min="0" max="100" value="<%= String.format("%.2f", s.getGrade()) %>">
